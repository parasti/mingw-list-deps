...
# If 'make install' is used to install this script, a few of the
# settings below are overwritten with the ones from the Makefile.

MINGW_PREFIX=~/dev/mingw
export MINGW_PREFIX

# This isn't plain 'wine' because in Debian the Wine in PATH is
# actually the Winelauncher script, which is a bit too slow and too
# helpful for our tastes.
...
# in most autotools-based packages.  You can probably use the script
...
# When a project that uses libtool does not inform libtool that it has
# been ported to build native Windows DLLs (by using the
...
# libraries fail to build.  Although libtool documentation does
# mention this requirement, libtool itself does not seem to handle
# this cleanly and fails in obscure ways.  Explicitly setting OBJDUMP
# is (sometimes) a sufficient work-around.  (Note that this might have
